...
from data.datasets import get_dataset, ConcatDataset, SegDataset
from data.transform import RandomImgAugment, DepthToTensor
from data.seg_transform import SegRandomImgAugment
...
def create_test_dataloader(args):
    ...
    dataset = get_dataset(root=(args.root + '/kitti'), data_file=args.test_datafile, phase='test',
...
def create_train_dataloader(args):
    ...
    src_dataset = get_dataset(root=(args.root + '/vkitti'), data_file=args.src_train_datafile, phase='train',
    ...
    tgt_dataset = get_dataset(root=(args.root + '/kitti'), data_file=args.tgt_train_datafile, phase='train',
...
def create_seg_dataloader(args):
    joint_transform_list = [RandomImgAugment(True, True, True, args.loadSize)]
    img_transform_list = [ToTensor(), Normalize([.5, .5, .5], [.5, .5, .5])]
    ...
    dataset = SegDataset(args, SegRandomImgAugment())
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=args.batchSize, shuffle=True,
        num_workers=int(args.nThreads),
...
def create_dataloader(args):
    ...
    if args.model != 'seg':
        ...
        return create_seg_dataloader(args)
